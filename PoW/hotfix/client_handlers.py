...
class _ClientRequestHandler(Handler, metaclass=abc.ABCMeta):
    ...
    def __init__(self, request_proto, response_proto, response_type,
                 tree=None, local_tree=None, block_store=None):
    ...
    @property
    def _tree(self):
        """Gets the thread-local merkle tree.
        
        Note:
            This getter will fail if `_internal_tree` and `_local_tree` have not been set

        """
        local_tree = self._local_tree
        tree = self._internal_tree
        try:
            return local_tree.tree
        except AttributeError:
            local_tree.tree = copy.copy(tree)
            return local_tree.tree
    ...
    def _set_root(self, request):
        ...
        Note:
            This method will fail if `_internal_tree` and `_local_tree` have not been set
...
class StateListRequest(_ClientRequestHandler):
    ...
    def _respond(self, request):
        ...
        start = request.paging.start
        limit = min(request.paging.limit, MAX_PAGE_SIZE) or DEFAULT_PAGE_SIZE
        reverse = self.is_reverse(request.sorting, self._status.INVALID_SORT)
        ...
        entries = [
            ...
            for a, v in self._tree.leaves(request.address or '', start=start, limit=limit, reverse=reverse).items()]
...
class RewardListRequest(_ClientRequestHandler):
    ...
    def _respond(self, request):
        ...
        - is greater than the DEPTH_LIMIT, fetch the the first predecessor by number.
        - else, traverse from the head block up to the first pred requested.

        Returns: Blocks sorted in ascending order by block height. from [last pred to first pred].
        """
        if request.head_id:
            if self._id_regex.fullmatch(request.head_id) is None:
                LOGGER.debug('Invalid head id requested: %s', request.head_id)
