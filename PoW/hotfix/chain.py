...
class ChainController(object):
    ...
    def _verify_block_again(self,new_block, descendant_blocks = None):
            LOGGER.debug('Verify block again: %s ', new_block)
            self._blocks_pending[new_block.identifier] = descendant_blocks if descendant_blocks else []
            self._submit_blocks_for_verification([new_block])

    def _chain_changed_on_validation(self, result, descendant_blocks):
        ...
        if descendant_blocks:
            ...
            self._verify_block_again(new_block)
        ...
        try:
            with self._lock:
                ...
                if result.chain_head.identifier !=\
                    ...
                    self._chain_changed_on_validation(result, descendant_blocks)

                # If the head is to be updated to the new block.
                elif commit_new_block:
                    if self._chain_head_lock.acquire(timeout=self._PUBLISHER_TIMEOUT):
                        try:
                        ...
                        self._verify_block_again(new_block, descendant_blocks)
