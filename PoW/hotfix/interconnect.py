...
class _SendReceive(object):
    ...
    def send_last_message(self, msg, connection_id=None, callback=None, one_way=False):
        ...
        zmq_identity = None
        with self._connections_lock:
            LOGGER.debug("@SLM SR {}".format(self._connections))
            LOGGER.debug("@SLM SR {}".format(connection_id))
            if connection_id is not None and self._connections is not None:
                if connection_id in self._connections:
                    connection_info = self._connections.get(connection_id)
                    del self._connections[connection_id]
...
class Interconnect(object):
    ...
    def _get_connection(self, endpoint, conn_type):
        connection_id = None
        with self._connections_lock:
            for cid in self._connections:
                conn_info = self._connections[cid]
                if conn_info.uri == endpoint and conn_info.connection_type == conn_type:
                    connection_id = cid
                    break
            return connection_id

    def _get_outbound_connection(self, endpoint):
        return self._get_connection(endpoint, ConnectionType.OUTBOUND_CONNECTION)

    def _get_inbound_connection(self, endpoint):
        return self._get_connection(endpoint, ConnectionType.ZMQ_IDENTITY)
    ...
    def allow_inbound_connection(self):
        ...
        LOGGER.debug("@AIC {}".format(self._connections))
        ...
        return self._max_incoming_connections >= connection_count
    ...
    def get_outbound_connection_id_by_endpoint(self, endpoint):
        """Returns the connection id associated with a publically
        reachable endpoint or raises KeyError if the endpoint is not
        found.
    ...
    def is_outbound_connection(self, connection_id):
        with self._connections_lock:
            connection_info = self._connections.get(connection_id)
        if connection_info and connection_info.connection_type == \
                ConnectionType.OUTBOUND_CONNECTION:
            return True
        return False
