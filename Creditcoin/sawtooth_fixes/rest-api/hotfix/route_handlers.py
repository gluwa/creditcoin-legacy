...
class RouteHandler(object):
    ...
    async def fetch_block(self, request):
        ...
        by_id = True
        if not self._check_id(block_id):
            by_id = False
            if not re.fullmatch('[1-9][0-9]*', block_id):
                raise errors.InvalidResourceId(block_id)
        ...
        if by_id:
            ...
            response = await self._query_validator(
                ...
                client_block_pb2.ClientBlockGetResponse,
                client_block_pb2.ClientBlockGetByNumRequest(block_num=int(block_id)),
                error_traps)
